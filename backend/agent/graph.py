...
def build_agent_graph(
    ...
    workflow.add_node("repository_search", make_repository_search_node(vector_store, embedding_provider))
    workflow.add_node("graph_search", make_graph_search_node(neo4j_client, vector_store))
    workflow.add_node("code_analyzer", code_analyzer_node)
    workflow.add_node("decision", decision_node)
    workflow.add_node("answer", make_answer_node(llm_provider))
    workflow.add_node("test", make_test_node(test_runner))
    workflow.add_node("propose_change", make_propose_change_node(vector_store, embedding_provider, llm_provider))
    workflow.add_node("human_approval", human_approval_node)
    workflow.add_node("apply_change", make_apply_change_node(vector_store, embedding_provider, llm_provider))
    workflow.add_node("run_tests_after_apply", make_run_tests_after_apply_node(test_runner))
