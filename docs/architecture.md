...
User-facing interface for interacting with the agent: submitting repositories, asking questions, reviewing proposed changes, and approving or rejecting modifications. No framework has been chosen or initialized yet; `frontend/` remains empty. Vercel is the documented target platform if/when a Next.js frontend is built, rather than a hand-rolled container — see `docs/deployment.md`'s "Frontend Deployment" section.

### Backend API
**Status: PARTIALLY IMPLEMENTED (Phase 15)**

`backend/api/` hosts a real, minimal FastAPI application (`main.py`, `health.py`) exposing `/`, `/health/live`, and `/health/ready` — built for production deployment (see `docs/deployment.md`), not as a demo. It deliberately does **not** yet expose the agent's actual operations (asking a question, approving a change, running an evaluation, integrating a GitHub repository) as HTTP endpoints — those remain plain Python libraries (`agent/service.py`, `github_integration/service.py`, `evaluation/suite.py`), invoked directly from tests and manual demo scripts, exactly as before this phase. Adding an authorized HTTP surface over those operations — preserving every Phase 14 approval/authorization gate — is future, separately-scoped work; the health/readiness/config-validation layer built in Phase 15 does not need it and was not blocked by its absence.
...
### Security
**Status: IMPLEMENTED (Phase 14 — a comprehensive audit and hardening pass across Phases 1-13; two real vulnerabilities found and fixed, several controls re-verified against real adversarial input)**

Full detail — threat model, trust boundaries, per-phase findings, residual risks, and security assumptions — lives in **[docs/security.md](security.md)**, not duplicated here. Summary: repository content (source, comments, README, config, GitHub metadata, tool/test output) is treated as untrusted data end to end; no consequential action (writing a file, pushing a branch, opening a pull request) is ever derived from parsing that content or an LLM's output — only from an explicit, externally-supplied approval. Two real gaps were found by adversarial testing and fixed: (1) a target repository's committed secrets (`.env`, private keys) were not excluded from RAG indexing or direct file-read tools — now excluded at both chokepoints (`ingestion/filters.py`, `tools/file_tools.py`); (2) `ModificationService.apply_change` had no approval check of its own, relying entirely on the calling agent node — it now requires an explicit `approved=True` keyword argument, matching the pattern `GitHubIntegrationService` already used. Docker sandbox isolation and GitHub credential handling were re-verified against real containers and real `git`/URL-parsing edge cases (a URL-validation gap involving control characters and legacy `;params` syntax was also found and fixed). Prompt injection is explicitly NOT claimed to be solved — see `docs/security.md` section 5 for what is and isn't defended. Covered by `tests/security/` (88 tests).

### Deployment
**Status: PARTIALLY IMPLEMENTED (Phase 15 — production infrastructure built and verified locally; no cloud deployment performed)**

Full detail — target production architecture, what's implemented/tested vs. only a documented target, secrets categorization, health checks, CI, and honest dependency-vulnerability-scan results — lives in **[docs/deployment.md](deployment.md)**, not duplicated here. Summary: a minimal FastAPI app (`backend/api/`) exposes liveness/readiness health checks (Neo4j outages never block readiness, since Neo4j is optional); a production Docker image (`backend/Dockerfile`) was built and run locally as a non-root user with no baked-in secrets; `.github/workflows/ci.yml` runs the full test/security/type-check suite plus both Docker builds on every push/PR, with no auto-deploy job since no real deployment target is provisioned. The single most important architectural rule this phase reinforces: the application container (holds real credentials) and the Phase 10 sandbox container (executes untrusted repository code) are, and must remain, two separate images with no shared runtime and no Docker-socket access between them.
...
- **Phase 14: Security audit & hardening** — see the "Security" section above and, for full detail, `docs/security.md`. Covered by `tests/security/` (88 adversarial regression tests against the real, integrated system — real symlinks, a real fork bomb inside a real container, real `git` ref validation, real redaction through a real traced span). Two real vulnerabilities were found and fixed (repository secrets not excluded from RAG/tool access; `ModificationService.apply_change` missing its own approval check), plus a GitHub URL-validation hardening (control characters / legacy `;params` syntax). No Phase 1-13 behavior was changed except where a genuine security issue required it.
- **Phase 15: Production deployment** (`backend/api/`, `backend/config.py`, `backend/Dockerfile`, `.github/workflows/ci.yml`) — see the "Deployment" section above and, for full detail, `docs/deployment.md`. Covered by an automated test suite in `tests/api/` and `tests/test_config.py` (21 tests: liveness/readiness behavior under healthy/degraded/misconfigured scenarios, production error handling never leaking secrets/paths, CORS defaults, config validation never exposing values). The production Docker image was built and run locally against a real PostgreSQL instance, verified non-root with no baked-in secrets and architecturally separate from the Phase 10 sandbox image (no Docker/git CLI, no socket). `pip-audit` was run for real (26 known vulnerabilities found, mostly transitive; documented rather than fixed this phase, per the project's own "avoid breaking changes this late" policy). No cloud deployment was performed or is claimed.
